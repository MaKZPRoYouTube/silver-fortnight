...
export interface ValidationResult {
  ...
  metrics: {
    ...
    requiredHorizontalDistance: number;
    maxHorizontalReach: number;
    margin: number;
    simulatedLandingX: number;
  };
}

export class PhysicsValidator {
  constructor(private readonly physics: PhysicsParams = DEFAULT_PHYSICS) {}

  validateSituation(situation: SituationData): ValidationResult {
  ...
  private aggregateWind(modifiers: ModifierData[]): {
    acceleration: number;
    ...
    for (const modifier of modifiers) {
      if (modifier.type !== 'WIND') continue;
