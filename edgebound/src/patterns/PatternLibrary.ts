...
export interface PatternRecipe {
  ...
  tags: string[];
  weight: number;
  createsRecovery: boolean;
  requiresPrior?: PatternType[];
  forbidsAfter?: PatternType[];
  build(rng: SeededRandom, tier: DifficultyTier): PatternData;
}

const common = (tier: DifficultyTier) => ({
  startX: 180,
  startY: 410,
  targetX: 520,
  targetY: 350,
...
export const PATTERN_LIBRARY: readonly PatternRecipe[] = [
  {
...
export function getRecipe(type: PatternType): PatternRecipe {
  const recipe = PATTERN_LIBRARY.find((r) => r.type === type);
  if (!recipe) throw new Error(`Unknown pattern: ${type}`);
  return recipe;
}

export function spawnPatternRuntime(data: PatternData, rng?: SeededRandom): PatternRuntime {
  void rng;
  return spawnPattern(data);
}

export function getAvailableRecipes(tier: DifficultyTier): PatternRecipe[] {
