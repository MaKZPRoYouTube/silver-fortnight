...
import { SituationData } from './core/types.js';
...
function finish(): void {
  ...
  if (runtime.state === 'SUCCESS') {
    score += result.coins;
    if (perfect) {
      slowMo = .35; shake = 8; showFeedback('PERFECT!');
      burst(runtime.player.getCenterX(), runtime.player.getBottom(), '#ffd372', 28); playTone(660, .25, 'sine');
    } else if (good) {
      showFeedback('GOOD'); burst(runtime.player.getCenterX(), runtime.player.getBottom(), '#81d9d0', 16); playTone(460, .16, 'sine');
    } else {
      showFeedback('EDGE HIT'); burst(runtime.player.getCenterX(), runtime.player.getBottom(), '#91a2c8', 12); playTone(360, .12, 'triangle');
    }
    ...
    shake = 10; showFeedback('MISSED'); playTone(115, .3, 'sawtooth');
    resultKicker.textContent = 'RUN FAILED'; resultTitle.textContent = 'TRY AGAIN.'; resultReward.textContent = 'SAME WIND. BETTER TIMING.';
  }
...
function draw(): void {
  const width = canvas.clientWidth, height = canvas.clientHeight; context.clearRect(0, 0, width, height);
  ...
  const cameraX = Math.max(0, runtime.player.getCenterX() - width * .34); const baseY = height - 165; const offsetX = (Math.random() - .5) * shake;
  ...
  for (const platform of runtime.platforms) {
    ...
    context.shadowBlur = isTarget ? 18 : 0; context.shadowColor = '#4ddbc9';
    context.fillStyle = isTarget ? '#77d9ce' : '#5379a2'; context.fillRect(platform.x, platform.y, platform.width, platform.height);
    context.shadowBlur = 0; context.fillStyle = '#182849'; context.fillRect(platform.x + 5, platform.y + 5, platform.width - 10, platform.height - 10);
    if (isTarget) { context.fillStyle = '#ffd372'; context.fillRect(platform.x + platform.width / 2 - perfect / 2, platform.y + 5, perfect, 4); }
    context.fillStyle = 'rgba(255,255,255,.55)'; context.fillRect(platform.x, platform.y, platform.width, 2);
  }
